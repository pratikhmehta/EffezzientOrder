...
{
    NSString *regex = @"[A-Z0-9a-z._%+-@*]{5,}+";
    NSPredicate *passTestPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    return [passTestPredicate evaluateWithObject:self];
}
...
{
//    NSString *regex = @"[23456789][0-9]{6}([0-9]{3})?";
...
- (NSString *)getFirstNCharacters:(NSInteger)numberOfCharacters{
    NSString *prefix = nil;
    
    if ([self length] >= numberOfCharacters)
        prefix = [self substringToIndex:numberOfCharacters];
    else
        prefix = self;
